...
This tutorial will walk you through how we built zkDrop, an application allowing whitelisted wallets and GitHub accounts to benefit from an airdrop on a public address while keeping their eligible account private thanks to an on-chain [**sismoConnect**](../../what-is-sismo/sismoconnect.md) integration.
...
The repository use the [`@sismo-core/sismo-connect-react`](https://docs.sismo.io/sismo-docs/technical-documentation/sismoconnect/sismoconnect-react-request) package to request the proof and the [sismoConnect Solidity library](../../technical-documentation/sismoconnect/sismoconnect-solidity-library-verify-on-chain-soon.md) to verify it.
{% endhint %}
...
{% hint style="info" %}
We definitely recommend our entry-point tutorial for a quick off-chain sismoConnect integration [here](authenticate-your-users-with-sismoconnect.md). Feel free to take a look or even try it first before following this more advanced one.&#x20;
{% endhint %}
...
You can find the complete example of a Next.js repository setup with sismoConnect [here](https://github.com/sismo-core/zkdrop).
...
<figure><img src="../../.gitbook/assets/Capture d’écran 2023-03-14 à 19.47.52 (2).png" alt=""><figcaption><p>Register your sismoConnect App in the <a href="https://factory.sismo.io/apps-explorer">Sismo Factory</a></p></figcaption></figure>
...
Congratulations! You now have an appId (`0x11b1de449c6c4adb0b5775b3868b28b3`) and a groupId (`0x42c768bb8ae79e4c5c05d3b51a4ec74a`), which means you can integrate [sismoConnect](../../what-is-sismo/sismoconnect.md) into your app. Let’s now focus on how to generate and verify the proof.
...
To easily generate a proof for these three requests, you can use the [`@sismo-core/sismo-connect-react`](../../technical-documentation/sismoconnect/sismoconnect-react-request.md) package or the [@sismo-core/sismo-connect-client](../../technical-documentation/sismoconnect/sismoconnect-client-request.md) package.
...
To do so, you will first have to create a `sismoConnectConfig` with the appId you registered in the Factory.

<pre class="language-typescript"><code class="lang-typescript">import { AuthType, useSismoConnect, SismoConnectButton, SismoConnectClientConfig } from "@sismo-core/sismo-connect-react";

<strong>export const sismoConnectConfig: SismoConnectClientConfig = {
...
You can see the [documentation](../../technical-documentation/sismoconnect/sismoconnect-client-request.md) if you want to learn more about Claim requests. You can also learn more about Auth requests that can be requests of Data Vault ownership but only Twitter or GitHub account ownership for example.
{% endhint %}



{% hint style="success" %}
If you are not part of the group, feel free to use the optional `devMode` in your sismoConnectConfig. Set the boolean field enabled as true and input a list of addresses that will be used instead of the actual group. Beware that this devMode should only be used when prototyping, if you keep such config in production, these dev addresses will be able to make proofs in their Vaults.
{% endhint %}
...
You can use the `useSismoConnect` react hook to get the response back as a typescript object and as bytes. The typescript object can be used to see the response in clear while the responseBytes is the [`sismoConnectResponse`](../../technical-documentation/sismoconnect/sismoconnect-solidity-library-verify-on-chain-soon.md) encoded as bytes that you will send to your smart contracts.
...
To be able to use the [sismoConnect Solidity library](../../technical-documentation/sismoconnect/sismoconnect-solidity-library-verify-on-chain-soon.md), you will first need to install Foundry.&#x20;
...
You can now put your contracts under `contracts/src` and start to use the [sismoConnect Solidity Library](../../technical-documentation/sismoconnect/sismoconnect-solidity-library-verify-on-chain-soon.md).&#x20;
...
contract ZKDropERC721 is ERC721, SismoConnect {
  ...
  SismoConnectRequestContent private sismoConnectRequestContent;
...
```solidity
// the function takes the sismoConnectResponse as bytes 
// and the `to` address on which the user wants to claim the ERC721
function claimWithSismoConnect(bytes memory sismoConnectResponse, address to) public {
    ...
    SismoConnectVerifiedResult memory sismoConnectVerifiedResult = verify({
        // the sismoConnectResponse as bytes
    ...
    uint256 tokenId = sismoConnectVerifiedResult.verifiedAuths[0].userId;
